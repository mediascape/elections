...
'mediascape/AdaptationToolkit/adaptation/multiDeviceAdaptation/plugins/hbbtv'
],
function($, applicationContext){
   ...
   if (statusBefore.length>0 )
    ...
    var onUpdateContext = function (change) {
      ...
      var needInfoReady = context.agents.every (function(ag){
        if ( ag.capabilities.hasOwnProperty('screenSize')) return true;
        else return false;
      });
  ...
  var loadJSONRules = function(file) {
    ...
    $.getJSON(file+"?"+new Date().getTime(), function(rules){
  ...
  this.startApplicationContext = function (){
    ...
    map.getGroupMapping(_this.GROUP_ID).then(function (data1) {
      ...
      console.log("mappingService working!!")
      local_agent_id = applicationContext.getAgents().self.agentid;
  ...
  this.getAppAttribute= function(key){
      return applicationContext.getItem(key);
  },
