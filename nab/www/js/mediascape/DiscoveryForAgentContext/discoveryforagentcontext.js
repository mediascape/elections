				...
				var instrument = {
					...
					on: function () {
						mediascape.discovery.getExtra("screensize").then(function(data){
							...
							else if(navigator.userAgent.toLowerCase().indexOf('hbbtv')!=-1){
									data.extra[1].screenX = "24";
									data.extra[1].screenY = "18";
									data.extra[0].width = "20000";
									data.extra[0].height = "20000";									
									ac.setItem("screenSize", data.extra);
							}
		...
		var componentStatusInstrument = {
				init: function () {
					...
					document.addEventListener('onComponentsChange',function(data){
						...
						document.removeEventListener('onComponentsChange', arguments.callee);
