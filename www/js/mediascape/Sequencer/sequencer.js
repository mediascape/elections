...
define(["./motionutils", './timeoututils', './interval', './axis'], 
	function (motionutils, Timeout, Interval, axis)  {

	'use strict';

	// UTILITY
	// sequencer works on a clock counting seconds.
	var secClock = motionutils.secClock, 
		P = motionutils.P,
		V = motionutils.V,
		A = motionutils.A,
		T = motionutils.T;
	...
	*/
	...
	Sequencer.prototype._onMotionUpdate = function (event) {
	    ...
	    */
	    var isMoving = motionutils.isMoving(initVector);
	    if (isMoving) {
		    ...
		    points.forEach(function (pointInfo) {
				...
				if (pointInfo.pointType === axis.PointType.SINGULAR) {
					...
					var direction = motionutils.calculateDirection(initVector, now);
					var entering = true;						
	...
	Sequencer.prototype.updateAll = function(argList) {
	    ...
	    var now = secClock();
	    var nowVector = motionutils.calculateVector(this._motion.getInfo().vector, now);
	    var nowPos = nowVector[P];
		...
		*/
		var isMoving = motionutils.isMoving(nowVector);
		if (!isMoving) {
	...
	*/
	Sequencer.prototype._main = function (now) {
        ...
        var isMoving = motionutils.isMoving(this._motion.getInfo().vector);
	...
	Sequencer.prototype._load = function (now, givenPoints) {
	    ...
	    */
	    ...
	    if (!points) {
			...
			var posInterval = motionutils.calculateInterval(vectorStart, tDelta);
	    ...
	    */
	    ...
	    var eventList = motionutils.calculateSolutionsInInterval(vectorStart, 
								 tDelta,
								 points);
	    /* 
	    ...
	    */	    
	    var rDelta = motionutils.calculateDelta(vectorStart, rangeInterval)[0];
	    ...
	    eventList.forEach(function (e)  {
			...
			*/
			if (task.pointType === axis.PointType.LOW || task.pointType === axis.PointType.HIGH) {
			    var v = motionutils.calculateVector(this._motion.getInfo().vector, tStart + d);
			    if (v[V] === 0){
					push = false;
			    }
			}
	...
	*/
	...
	/* 
		Works on timed data which will not change and stored in an array
		Uses array index as unique key.
		Uses "start" and "end" properties in array elements for intervals 
	*/
	...
	ArraySequencer.prototype.loadData = function () {
		...
		for (var i=0; i<this._array.length; i++) {
			r.addCue(i.toString(), new Interval(this._array[i].start, this._array[i].end), this._array[i]);
		}
		r.submit();
	};
